...
console.log('Digite seu sexo (m ou f)');
let sexoUsuario = prompt();
if (sexoUsuario !== 'm' && sexoUsuario !== 'f') {
  console.log('Digite apenas m para masculino ou f para feminino');
  return;
}
...
if (isNaN(altura)) {
  console.log('Digite uma altura válida em metros');
  return;
}

let pesoHomem = 52 + ((altura - 1.52) / 0.02) * 1.9;
let pesoMulher = 49 + ((altura - 1.52) / 0.02) * 1.7;

if (sexoUsuario == 'm') {
  console.log(`${nome}, seu peso ideal é: ${pesoHomem} kg`);
} else {
  console.log(`${nome}, seu peso ideal é: ${pesoMulher} kg`);
}
